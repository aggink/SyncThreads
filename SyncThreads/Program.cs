 ...
 * При этом, хоть потоки и запускаются асинхронно параллельно, 
 *  но очередь их доступа к шаренному ресурсу (по факту к началу выполнения связанного с ним делегата) должна описываться и настраиваться в координаторе: 
 *  в каждый конкретный момент времени только один поток будет осуществлять работу с шаренным объектом. 
 * К примеру: делегат первого потока, выполняющийся первым, создает пустой документ. 
 * Последний, третий, поток выполняет делегат сохранения документа в файл. 
